...
const DEFAULT_TEXT_CHARS_PER_FILE = 2000;
const MAX_BATCH_CONTEXT_KB = 8;
const MAX_FILES_PER_BATCH = 10;
const MODEL_RESPONSE_TIMEOUT_MINUTES = 5;
const initialSort: SortSettings = { mode: "standard", customPrompt: "", textLimit: DEFAULT_TEXT_CHARS_PER_FILE, totalLimit: 500000, unlimited: false };
...
const formatBytes = (bytes?: number) => bytes == null ? "" : bytes < 1024 ? `${bytes} Б` : `${(bytes / 1024).toLocaleString("ru-RU", { maximumFractionDigits: 1 })} КБ`;
...
const formatAnalysisLog = (event: AnalysisLogEvent) => {
  ...
  return `${phase} ${event.batchNumber}/${event.totalBatches} · попытка ${event.attempt}/2 · ${event.fileCount} файлов · расширения: ${formatExtensions(event)}${event.inputBytes == null ? "" : ` · контекст: ${formatBytes(event.inputBytes)}`} · ${formatDuration(event.durationMs)} · ${outcome} · успешно: ${event.successfulFiles} · ${unresolvedLabel}: ${event.unresolvedFiles}${event.errorDetail ? ` · ${event.errorDetail}` : ""}`;
};
...
function App() {
  ...
  const analyze = async () => {
    ...
    setAnalyzing(true); setCancelRequested(false); setProgress(null); setStatus("Идёт анализ"); writeLog(`Запуск анализа · до ${MAX_FILES_PER_BATCH} файлов и до ${MAX_BATCH_CONTEXT_KB} КБ контекста на запрос · ожидание ответа: до ${MODEL_RESPONSE_TIMEOUT_MINUTES} мин. · максимум попыток: 2 · режим: ${sort.mode === "standard" ? "стандартный" : "кастомный"} · провайдер: ${ai.provider}`); writeLog("Сканирование файлов и подготовка безопасного плана…");
